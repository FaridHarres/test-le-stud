...
const Activities = ({ date, user, project }) => {
  ...
  const onAddActivities = (project) => {
    const found = activities.find((a) => a.project === project);
    if (found) return toast.error(`Project ${project} already added !`);
    setActivities([
      ...activities,
      {
        project,
  ...
  async function onSave() {
    for (let i = 0; i < activities.length; i++) {
      ...
      toast.success(`Saved ${activities[i].project}`);
    }
  }
                ...
                <tbody>
                  ...
                  <tr>
                    <th className="w-[50px] text-[12px] text-[#212325] px-[10px] py-2">
                      <SelectProject disabled={activities.map((e) => e.project)} value="" onChange={(e) => onAddActivities(e.name)} />
                    </th>
                  </tr>
                </tbody>
              </table>
