      ...
      <div className="flex flex-wrap p-3">
        <div className="w-full ">
          <div className="flex gap-3">
            <div>{project.logo && <img className="w-[95px] h-[85px] border border-[#E5EAEF] rounded-[8px] object-contain" src={project.logo} alt="ProjectImage.png" />}</div>
            <div className="w-full">
...
const Budget = ({ project }) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data } = await api.get(`/activity?project=${encodeURIComponent(project.name)}&date=${dateQuery}${date.getTime()}`);
      setActivities(data);
    })();
  }, []);
...
const Activities = ({ project }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      date_to.setDate(0);
      const { data } = await api.get(`/activity?dateFrom=${from.getTime()}&dateTo=${date_to.getTime()}&project=${encodeURIComponent(project.name)}`);
      setActivities(data);
    })();
  }, [date, project]);
                          ...
                          <tr className="border-t border-b border-r border-[#E5EAEF]" key={`1-${e._id}`}>
                            <th className="w-[100px] border-t border-b border-r text-[12px] font-bold text-[#212325] text-left">
                              <div className="flex flex-1 items-center justify-between gap-1 px-2">
                                ...
                                <div className="text-md italic font-normal">{(e.total / NB_HOURS_WORKING).toFixed(2)} days</div>
                              </div>
                            </th>
                            {e.detail.map((f, j) => {
  ...
  return (
    <div className="flex flex-wrap gap-3">
      ...
      {project.links.map((link, i) => (
        <div key={i} className="group text-sm font-medium	text-blue-700 border-[1px] border-blue-700 rounded-full overflow-hidden">
